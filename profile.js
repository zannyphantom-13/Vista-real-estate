...
document.getElementById('profileUpload')?.addEventListener('change', async (e) => {
    ...
    try {
        ...
        if (!cloudinaryRes.ok) throw new Error('Cloudinary explicitly rejected the Avatar payload structure.');
        ...
        showToast('Trust Avatar successfully updated! Please wait while the environment seamlessly reloads.', 'success');
        setTimeout(() => window.location.reload(), 1500);
    } catch(err) {
        showToast('Avatar Injection mathematically failed: ' + err.message, 'error');
    }
});
